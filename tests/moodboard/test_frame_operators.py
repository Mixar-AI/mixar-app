...
def test_f2_reaches_a_selected_frame_before_a_reference():
    """A frame is selected by its own border, so a selected frame is an
    unambiguous F2 target."""
    source = _read(MOODBOARD / "ui/operators/node_graph_ops.py")
    body = source.split("if not self.node_id:")[1].split("\n\n")[0]
    assert body.index("moodboard_rename_frame") < body.index("moodboard_rename_media")
