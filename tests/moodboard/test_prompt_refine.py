...
def test_the_node_tile_draws_refine_and_revert_from_those_props():
    ...
    assert source.count('RNA_string_set(UI_but_operator_ptr_ensure(refine), "node_id"')
...
def test_the_node_tile_keeps_refine_beside_revert():
    ...
    assert 'RNA_string_set(UI_but_operator_ptr_ensure(refine), "node_id"' in source
    assert 'RNA_string_set(UI_but_operator_ptr_ensure(revert), "node_id"' in source
