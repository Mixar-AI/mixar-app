...
def test_the_panel_says_why_the_gyro_is_missing():
    ...
    panel = (
        ROOT
        / "src/scripts/mixar/modules/virtual_camera/ui/panels/virtual_camera_panel.py"
    ).read_text(encoding="utf-8")
    assert "if not state.tls:" in panel
    assert "No TLS: joystick control only" in panel
    # And the server states it ONCE: it records the flag the panel reads and
