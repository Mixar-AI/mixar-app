...
def test_the_node_tile_only_offers_a_mic_where_a_prompt_exists():
    ...
    The mic shares its `show_prompt` branch with Refine / Revert, which act on
    the same field and are gated by the same thing — so this asserts the guard
    ENCLOSES the mic by walking braces, rather than measuring the distance
    between them. A character-distance check said the same thing until the
    refine pair landed between the two and pushed them apart.
    """
    ...
    assert guard_at > 0
    # From the guard's opening brace to the mic, every block that opens must
    # also close: if the branch had ended before the mic, depth would go
    # negative on the `}` that closed it.
    depth = 0
    for char in tile[tile.index("{", guard_at):mic_at]:
        if char == "{":
            depth += 1
        elif char == "}":
            depth -= 1
            assert depth > 0, "the mic sits outside the show_prompt branch"
    assert depth > 0
