...
{
  inst_.visible_objects.begin_sync();
...
{
  inst_.visible_objects.sync_object(ob, res_handle);
  const eViewLayerEEVEEPassType enabled_passes = inst_.film.enabled_passes_get();
  if (!inst_.visible_objects.enabled() && !(enabled_passes &
        (EEVEE_RENDER_PASS_CRYPTOMATTE_OBJECT | EEVEE_RENDER_PASS_CRYPTOMATTE_ASSET)))
  {
    return;
  }
...
{
  inst_.visible_objects.sync_material(material);
...
{
  inst_.visible_objects.end_sync();
