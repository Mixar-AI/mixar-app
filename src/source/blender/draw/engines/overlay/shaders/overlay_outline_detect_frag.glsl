...
void main()
{
  ...
  if (ref_col == 0u) {
    ...
    /* MIXAR: selection-suggestion ghost outline. Dimmed and screen-space
     * dashed (marching-ants style) so it reads as a transient preview, never
     * as an actual selection — full-strength solid silhouettes were mistaken
     * for the app selecting objects on its own. */
    float dash = step(7.0f, mod(gl_FragCoord.x + gl_FragCoord.y, 14.0f));
    frag_color = suggested_color * (0.06f + 0.74f * dash);
  }
  else if (color_id == 3u) {
