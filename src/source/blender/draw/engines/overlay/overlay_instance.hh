...
class Instance : public DrawEngine {
 ...
 private:
  /* MIXAR: objects ghost-outlined as selection suggestions. Rebuilt each
   * begin_sync() from the Python-owned scene property (see
   * mixar_sync_suggested_outline_names). */
  Set<std::string> mixar_suggested_names_;

  void mixar_sync_suggested_outline_names();
  bool mixar_object_is_suggested(const ObjectRef &ob_ref) const;
