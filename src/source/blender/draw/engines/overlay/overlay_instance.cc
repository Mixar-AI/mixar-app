...
void Instance::begin_sync()
{
  ...
  /* MIXAR: refresh the selection-suggestion ghost outline set before any
   * object_sync sees it. */
  mixar_sync_suggested_outline_names();
...
void Instance::object_sync(ObjectRef &ob_ref, Manager &manager)
{
  ...
  if (!state.hide_overlays) {
    ...
    /* MIXAR: suggested (ghost) outlines share the selection outline pass.
     * A real selection always wins over a stale suggestion. */
    const bool mixar_suggested = !object_is_selected(ob_ref) &&
                                 mixar_object_is_suggested(ob_ref);
    if ((object_is_selected(ob_ref) || mixar_suggested) && !in_edit_paint_mode) {
      outline.object_sync_ex(manager, ob_ref, resources, state, mixar_suggested);
    }
  }
}
...
/* MIXAR: selection-suggestion ghost outlines.
 *
 * Python (modules/selection_assist) writes candidate object names into the
 * scene string property "mixar_suggested_outline" (registered bpy.props ->
 * stored as an IDProperty), joined by the unit-separator control char.
 * Reading it here every begin_sync keeps the bridge one-directional and
 * main-thread only; a redraw tag from Python is enough to refresh. Keep the
 * property name/separator in sync with selection_assist/constants.py. */
void Instance::mixar_sync_suggested_outline_names()
{
  mixar_suggested_names_.clear();

  if (state.scene == nullptr) {
    return;
  }
  const ID &scene_id = state.scene->id;
  const IDProperty *prop = nullptr;
  /* Registered python properties live in system_properties (5.0+); fall back
   * to user properties for robustness. */
  for (const IDProperty *group : {scene_id.system_properties, scene_id.properties}) {
    if (group != nullptr) {
      prop = IDP_GetPropertyTypeFromGroup(group, "mixar_suggested_outline", IDP_STRING);
      if (prop != nullptr) {
        break;
      }
    }
  ...
  for (const char *c = value;; c++) {
    if (*c == SEP || *c == '\0') {
      if (c > start) {
        mixar_suggested_names_.add(std::string(start, c - start));
      }
      if (*c == '\0') {
        break;
      }
...
bool Instance::mixar_object_is_suggested(const ObjectRef &ob_ref) const
{
  if (mixar_suggested_names_.is_empty()) {
    return false;
  }
  /* +2 skips the ID code prefix ("OB"). */
  return mixar_suggested_names_.contains(ob_ref.object->id.name + 2);
}
