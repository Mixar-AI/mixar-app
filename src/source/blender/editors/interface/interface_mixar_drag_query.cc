 ...
 * that is waiting to start a drag? #uiBut is private to this module, so the
...
bool UI_mixar_region_active_but_is_draggable(ARegion *region)
{
   ...
   * #UI_region_active_but_get, which falls back to #UI_BUT_LAST_ACTIVE and so
   * would keep naming a tile long after the cursor left it. */
  const uiBut *but = ui_region_find_active_but(region);
  return but != nullptr && ui_but_drag_is_draggable(but);
}
