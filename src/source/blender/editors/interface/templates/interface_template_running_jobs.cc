  ...
  for (Scene &scene : bmain->scenes) {
    ...
    if (WM_jobs_test(wm, &scene, WM_JOB_TYPE_RENDER)) {
      cancel_fn = [&scene](bContext &C) {
        /* Mixar: RE_RenderFrame chooses render_cancel from G.is_break. Stopping
         * only the WM job leaves that flag unset and delivers partial pixels
         * through render_complete. Stop the actual owner and mark cancellation. */
        G.is_break = true;
