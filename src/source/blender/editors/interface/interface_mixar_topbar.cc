...
void draw_slider_left(Button *but, const rcti *rect, const bool is_hover)
{
  ...
  mixar_card_fill_round(&thumb, rad, is_hover ? mixar_chrome::slider_thumb_hover : mixar_chrome::slider_thumb);
...
void draw_cinema_pill(Button *but, const rcti *rect, const bool is_hover, const bool is_active)
{
  ...
  if (lit) {
    ...
    float a[4];
    float b[4];
    mixar_card_to_float(mixar_chrome::cinema_pill_fill_on_b, a);
    mixar_card_to_float(mixar_chrome::cinema_pill_fill_on_a, b);
    for (int i = 0; i < 3; i++) {
  ...
  draw_label_gradient(rect,
                      but->drawstr.c_str(),
                      lit ? mixar_chrome::cinema_pill_label_b : mixar_chrome::cinema_pill_label_a,
...
void draw_viewport_pill(Button *but, const rcti *rect, const bool is_hover, const bool is_active)
{
  ...
  const bool lit = but->mixar_style.lit;
  const bool pressed = is_active || (but->flag & UI_SELECT) != 0;
  const float alpha = lit ? 1.0f : (pressed ? 0.85f : (is_hover ? 0.75f : mixar_chrome::viewport_pill_dim));
  ...
  uchar label[4];
  memcpy(label, lit ? mixar_chrome::viewport_pill_label_on : mixar_chrome::viewport_pill_label, sizeof(label));
...
void draw_profile_pill(Button *but, const rcti *rect, const bool is_hover, const bool /*is_active*/)
{
  ...
  mixar_card_draw_text(fs, &label_rect, but->drawstr.c_str(), mixar_chrome::profile_label, UI_STYLE_TEXT_LEFT);
}
