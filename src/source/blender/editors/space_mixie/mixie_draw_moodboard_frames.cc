                           ...
                           const bool collapsed)
{
  ...
  if (collapsed) {
    ...
    UI_draw_roundbox_4fv(&strip, true, MOODBOARD_FRAME_RADIUS * 0.5f, border);
    return;
  }
  ...
  UI_draw_roundbox_4fv(&strip, true, MOODBOARD_FRAME_RADIUS, border);
  UI_draw_roundbox_corner_set(UI_CNR_ALL);
}
...
void mixie_draw_moodboard_frame_labels(View2D *v2d, ARegion *region, PointerRNA *scene_ptr)
{
  ...
  const float view_scale = UI_view2d_scale_get_x(v2d);
