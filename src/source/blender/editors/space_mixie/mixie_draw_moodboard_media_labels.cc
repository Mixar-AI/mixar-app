...
#include "UI_resources.hh" /* ui::theme::get_color_4fv */
...
void mixie_draw_moodboard_selected_media_labels(const bContext *C,
                                                ...
                                                const MoodboardGraphCache *cache)
{
  /* The Zen drawer region includes a transparent tab gutter. Names are held
   * inside its painted canvas -- the same rect the node controls use -- so one
   * can never hide under the grip or spill into the viewport behind it. */
  const rcti canvas = moodboard_visible_canvas_rect(C);
  if (BLI_rcti_size_x(&canvas) < 44 || BLI_rcti_size_y(&canvas) < 24) {
    return;
  }
  ...
  ui::theme::get_color_4fv(TH_BACK, shadow_color);
  ...
  for (const int index : tiles.index_range()) {
    ...
    /* Pushed back inside the painted canvas when the anchor is off screen -- a
     * name drawn outside it is a name the user selected the media to read and
     * cannot. */
    const float left = float(canvas.xmin) + 4.0f;
    const float bottom = float(canvas.ymin) + 4.0f;
    text_x = std::clamp(
        text_x, left, std::max(left, float(canvas.xmax) - text_width - 4.0f));
    text_y = std::clamp(
        text_y, bottom, std::max(bottom, float(canvas.ymax) - line_height - 4.0f));
