 ...
 * Preview / Export row and a reference's Rename / Preview / Export row, so
 * the same glyph in the same place relative to the thing it belongs to means
 * the same thing on a card, on a reference and on a frame.
 *
 * One deliberate difference: this row is LEFT-aligned, beside the frame's
 * name, where the other two are right-aligned. The name is what the pencil
 * edits, and a frame's name lives at its top-LEFT corner -- putting the
 * button that changes it at the opposite end of a wide frame would separate
 * the control from its subject.
 *
 ...
 * through the ordinary text-edit cancel. Kept alive by #UI_but_active_only,
...
static void add_frame_card_actions(uiBlock *block,
                                   ...
                                   const char *frame_id)
{
  ...
  /* Laid out from the frame's RIGHT edge, like the other two rows, so the
   * buttons line up with a card's and a reference's when several things are
   * selected at once. The name keeps the left end of the same line. */
  ...
  uiBut *more = uiDefIconButO(block,
                              ButType::But,
                              "WM_OT_call_menu",
                              ...
                              height,
                              nullptr);
  RNA_string_set(UI_but_operator_ptr_ensure(more), "name", "MIXIE_MT_moodboard_frame");
  moodboard_set_node_tooltip(more,
                             "Frame options\n\nSelect contents, add the selection, "
                             "fit to contents, colour, lock, collapse, ungroup or delete.");
  x -= width + gap;

  /* The pencil sits where a card's Edit and a reference's Rename sit. A frame
   * has no settings, so what it edits is the name -- the label painted above
   * it, which this turns into a text field in place. */
  ...
  RNA_string_set(UI_but_operator_ptr_ensure(rename), "frame_id", frame_id);
