...
bool popup_data_get(bContext *C, DirectorPopupData *r_data)
{
...
uiBlock *popup_block_begin(bContext *C, ARegion *region, const char *name)
{
...
void popup_block_end(uiBlock *block)
{
  UI_block_direction_set(block, UI_DIR_DOWN);
  UI_block_bounds_set_normal(block, int(0.4f * UI_UNIT_X));
}

void popup_state(uiBut *but, const bool active, const bool enabled)
{
  if (active) {
...
void popup_section_label(uiBlock *block, const char *text, const int y, const int width)
{
  uiDefBut(block,
           ButType::Label,
           0,
...
{
  uiBlock *block = popup_block_begin(C, region, __func__);
  DirectorPopupData data;
  if (!popup_data_get(C, &data) || !data.camera) {
    popup_section_label(block, "No active shot camera", 0, UI_UNIT_X * 10);
    popup_block_end(block);
    return block;
  }
  ...
  for (int index = 0; index < 3; index++) {
    ...
    popup_state(but, data.camera->type == types[index].camera_type, data.editable);
  }
  y -= gap;
  ...
  if (data.camera->type == CAM_PERSP) {
    ...
    for (const LensPreset &preset : presets) {
      ...
      popup_state(but, std::abs(data.camera->lens - float(preset.mm)) < 0.5f, data.editable);
    }
    y -= gap + row_h;
    uiBut *slider = uiDefButR(block,
                              ...
                              0,
                              std::nullopt);
    popup_state(slider, false, data.editable);
  }
  else {
    y -= gap + row_h;
    ...
    uiBut *value = uiDefButR(block,
                             ...
                             0,
                             std::nullopt);
    popup_state(value, false, data.editable);
  }
...
uiBlock *aspect_popup_create(bContext *C, ARegion *region, void * /*arg*/)
{
  ...
  for (const AspectPreset &preset : presets) {
    ...
    popup_state(but, active, data.editable);
  }
...
uiBlock *moves_popup_create(bContext *C, ARegion *region, void * /*arg*/)
{
  ...
  for (const MovePair &pair : pairs) {
    y -= label_h;
    popup_section_label(block, pair.section, y, width);
    y -= row_h;
    ...
    popup_state(left, false, data.editable);
    ...
    popup_state(right, false, data.editable);
    y -= gap;
  }
  ...
  uiBut *spacing = uiDefButR(block,
                             ...
                             0,
                             std::nullopt);
  popup_state(spacing, false, data.editable);
  ...
  uiBut *handheld = uiDefIconTextButR(block,
                                      ...
                                      0,
                                      std::nullopt);
  popup_state(handheld, false, data.editable);
  uiBut *intensity = uiDefButR(block,
                               ...
                               0,
                               std::nullopt);
  popup_state(intensity, false, data.editable);
