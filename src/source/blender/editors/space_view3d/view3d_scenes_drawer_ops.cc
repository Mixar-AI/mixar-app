 ...
 * and set mirror the moodboard drawer; `click` and `hover` are the drawer's
 * own: they turn a press on a scene card into the Python scene-tab operators
 * (`mixie_chat.new_scene_tab` / `switch_scene_tab` / `close_scene_tab`).
 */
...
static void VIEW3D_OT_scenes_drawer_hover(wmOperatorType *ot)
{
