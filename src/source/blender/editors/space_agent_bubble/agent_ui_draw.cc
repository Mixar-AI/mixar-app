...
#include "agent_ui_draw_helpers.hh"
...
using namespace agent_ui_draw_helpers;
                               ...
                               const AgentIslandState *state)
{
   ...
   * cat on a tier-colored gradient chip (Frame 1533210248.svg, mascot in
  ...
  if (w > h * 4.0f) {
    ...
    const MixieCatStyle &cat_style = mixie_cat_style_for_tier(state->subscription_type);
    const auto chip_a = mixie_cat_chip_color(cat_style.chip_shadow, pulse);
    const auto chip_b = mixie_cat_chip_color(cat_style.chip_light, pulse);
    ...
    fill_round_gradient(&chip, chip_r, chip_a.data(), chip_b.data(), chip_grad_a, chip_grad_b);
    ...
    agent_ui_draw_pill_cat(&chip, cat_pose, state->cat_activity, state->subscription_type);
