...
#include "render_mixar_visibility.hh"
...
static wmOperatorStatus screen_render_exec(bContext *C, wmOperator *op)
{
  ...
  if (!render_mixar_visibility_validate(op, scene)) {
    return OPERATOR_CANCELLED;
  }
  ...
  RE_mixar_visibility_begin(re, scene, RNA_boolean_get(op->ptr, "capture_visible_objects"));
  ...
  RE_mixar_visibility_finish(re, cancelled);
...
{
  RenderJob *rj = static_cast<RenderJob *>(rjv);
  RE_mixar_visibility_finish(rj->re, G.is_break || (rj->stop && *rj->stop));
...
void RENDER_OT_render(wmOperatorType *ot)
{
  ...
  render_mixar_visibility_property(ot);
