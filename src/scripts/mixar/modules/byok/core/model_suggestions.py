...
def is_local(provider: str) -> bool:
    """True when ``provider`` is the client-side Local LLM option."""
...
def get_provider_items() -> list[tuple[str, str, str]]:
    ...
    "Local LLM" options (none are part of the backend catalog), so a user can
    pick any even offline / before the catalog loads. The cloud providers come
    first (the cached catalog list, or a sentinel while it loads).
    """
    if _provider_cache:
    ...
    for client_item in (OPENROUTER_PROVIDER_ITEM, CODEX_PROVIDER_ITEM, LOCAL_PROVIDER_ITEM):
