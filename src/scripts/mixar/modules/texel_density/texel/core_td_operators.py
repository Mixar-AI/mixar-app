...
def find_image_editor_area(context):
	"""Find IMAGE_EDITOR area, preferably in MIXAR_UV mode"""
	# First try to find IMAGE_EDITOR in MIXAR_UV mode
	for area in context.screen.areas:
		if area.type == 'IMAGE_EDITOR':
			sima = area.spaces.active
			if sima and hasattr(sima, 'mode') and sima.mode == 'MIXAR_UV':
				return area
	...
	def execute(self, context):
		...
		for obj in start_selected_obj:
			...
			with context.temp_override(**override_dict):
				...
				# Preserve original mode (MIXAR_UV or UV) - don't change it
				...
				# Restore original space mode (preserve MIXAR_UV if it was that)
