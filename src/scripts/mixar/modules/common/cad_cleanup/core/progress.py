...
def document(data, prefix=''):
    ...
    if data.get('checkpoint_filename'): links+='<p><a href="'+prefix+'snapshots/'+data['timestamp']+'/checkpoint.mixar">Saved inspection checkpoint</a></p>'
...
def checkpoint(run,payload=None):
    ...
    data={'timestamp':stamp,'reason':reason,'status':terminal,'revision':run['revision'],
        'reference':status,'coverage':reference.coverage_rows(run),'images':captured[:3]+captured[3:][-8:],
    ...
    if payload.get('save_checkpoint'):
        ...
        if result!={'FINISHED'} or not destination.is_file(): state.fail('checkpoint_save_failed','The inspection checkpoint could not be saved.')
