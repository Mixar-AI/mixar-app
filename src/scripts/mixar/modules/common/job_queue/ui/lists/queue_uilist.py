...
class MIXIE_UL_unified_queue(UIList):
    ...
    def draw_item(self, context, layout, data, item, icon, active_data,
        ...
        outer = second.split(factor=0.4, align=True)

        # Left 40% = bordered pill. Sentence case (as authored in
        # _FEATURE_LABEL) reads better in a chip than a shout-cased tag.
        pill_box = outer.box()
        pill_box.label(text=job_type)

        # Remaining 60%: split in half — empty gap, then the status word.
        rest = outer.split(factor=0.5, align=True)
        ...
        # Trailing gap becomes the inter-item margin. Blender's
        # ``template_list`` collapses trailing ``col.separator(...)``
        # inside ``draw_item`` — the item's rendered height doesn't grow.
        # An empty ``col.row()`` with ``scale_y > 0`` is a real widget,
        # not a hint, so its height is respected and shows as a gap
...
def draw_unified_queue_panel(layout, context):
    ...
    # Filter chips row (All / Active / Done / Failed)
    chip_row = layout.row(align=True)
    chip_row.scale_y = 1.1
    chip_row.prop_enum(pg, "filter_mode", 'ALL', text="All")
    chip_row.prop_enum(pg, "filter_mode", 'ACTIVE', text="Active")
    chip_row.prop_enum(pg, "filter_mode", 'DONE', text="Done")
    chip_row.prop_enum(pg, "filter_mode", 'FAILED', text="Failed")
