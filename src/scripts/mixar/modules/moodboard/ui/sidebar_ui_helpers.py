...
# Image preview widgets moved out for the 500-line rule and are re-exported
# here: a dozen drawers already import them from this module, and where the
# code lives is not their business.
from .sidebar_image_widgets import (  # noqa: F401
...
def draw_prompt_section(layout, prop_owner, label="Prompt",
                        ...
                        min_lines=2, max_lines=5, context=None, refine=True):
    """Boxed prompt input with label and optional action button. Returns col.

    Every sidebar prompt comes through here, so BOTH prompt helpers are added
    here rather than in each drawer: the dictation mic (needs `context`) and
    the Refine / Revert row (``prompt_refine_drawer``; a tab with no target
    entry gets no row). They sit on opposite sides of the field on purpose —
    the mic FILLS it so it rides the header, Refine / Revert TRANSFORM what is
    already there so they sit below it.
    """
