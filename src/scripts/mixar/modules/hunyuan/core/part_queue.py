...
def _on_queue_changed(queue: FeatureQueue) -> None:
    ...
    was_generating = bool(getattr(scene, "mixie_hunyuan_part_is_generating", False))

    if has_work and not was_generating:
        try:
            scene.mixie_hunyuan_part_is_generating = True
    ...
    if not has_work and was_generating:
        try:
            scene.mixie_hunyuan_part_is_generating = False
        except (AttributeError, TypeError):
            pass


def _get_part_queue() -> FeatureQueue:
    global _listener_attached
    queue = get_queue(FEATURE_HUNYUAN_PART)
