...
| **Scene routing per session** | Every scene has `mixie_session_id`. On dequeue, switch `bpy.context.window.scene` to the matching scene, execute, restore. All within one timer tick — no redraw, no visible flicker. |
...
- **`mixie_session_id` per scene** — backend addresses scripts to a specific session; plugin routes execution to the matching scene by this property. No session_id → no scene match → script silently misrouted.
