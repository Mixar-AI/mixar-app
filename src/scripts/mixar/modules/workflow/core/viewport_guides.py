...
"""Zen Mode viewport guides: floor grid + relationship lines as one switch.

The floating shading strip's neighbour chip flips the floor/ortho grid,
axis lines, and relationship lines together so the calm canvas stays one
decision rather than four overlay flags. The chip paints pressed when any
of those guides are visible; a click settles every flag onto the opposite
...
def toggle_guides(space) -> bool:
    """Flip grid guides and relationship lines together; return the new state."""
    overlay = getattr(space, "overlay", None)
    if overlay is None:
        return False
