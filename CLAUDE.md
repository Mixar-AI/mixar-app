...
| **space_mixie_chat** | Agent chat: WebSocket JSON-RPC commands and streaming, reconnect-resume, sandboxed script execution, `llm.request` local-LLM relay, project/global rules, @-mention autocomplete, feedback stars, export lane, batched choice wizard, message copy, paste, attachments, voice |
| **agent_bubble** | The Agent island: floating always-on-top chat window with tabs (Agent, 3D, Media, Gaussian Splat, Library, Queue), elongated pill resting state, outside-click collapse; shares ConnectionManager/message store with space_mixie_chat. Working Mixie uses an open-eyed circular eye roll, not a squint. |
...
| **moodboard** | Reference boards + node canvas (Zen Mode: sliding VIEW_3D overlay drawer), catalog-driven generation sidebar (Image Gen, AI Render, Model Gen, Texture Gen, Scene Gen, Character Parts, Retopology, UV Unwrap, Mesh Segment, Auto Rig, Video Gen, Video Upscale, World Labs + Queue), turnaround/multi-view image-to-3D, prompt Refine/Revert on every generation prompt, clipboard, scene recon, annotations (drawer draw + erase), SAM3 character components, Gaussian splats |
| **director** | Phase-zero camera directing as a viewport mode: native C++ Cinema Mode surface plus gate/popups/timeline over Python-owned operators (the W/A/S/D/Q/E camera nudge and the aerial-map / Aerial-mode camera placement are the two native C++ modals, `view3d_director_nudge.cc` / `view3d_director_place_camera.cc`; the right column's preview card is a live top-down map, `view3d_director_minimap*.cc`); mode-aware rotation-continuity filter (`core/rotation_curves.py`) on every key-writing path so beats never interpolate the long way round; a second, camera-first Export to Moodboard surface (topbar Render menu + Timeline / Dope Sheet View menus) renders the same Beauty/Clay/Depth guides from a plain natively keyframed camera, with no Shot and no Director session |
| **virtual_camera** | Phone-as-camera over the LAN: a QR-paired browser control app (no install, iOS-safe) served by a self-signed-HTTPS + stdlib RFC 6455 WebSocket server; the phone's gyro and on-screen joysticks drive the bound camera, and the camera-POV viewport streams back to it |
...
- **Custom C++ editor spaces**: `space_mixie_chat`, `space_agent_bubble`, `space_mixar_properties`, `space_mixar_layers`, `space_mixar_assets`, plus the native chat renderer (markdown, thinking visualization, hit testing, thumbnails) and View3D overlays (Director, agent panel, gizmos). Authentication is cross-platform keyring + a local OAuth PKCE callback server. **Editor Type menu**: `SPACE_AGENT_BUBBLE` and the five texturing spaces (`MIXAR_LAYERS`, `MIXAR_PROPERTIES`, `MIXAR_ASSETS`, `BAKING`, `TEXTURE_SETS`) stay registered for saved layouts and `bl_space_type`, but `rna_Area_ui_type_itemf` hides them from the dropdown — same skip list as `SPACE_TOPBAR` / `SPACE_STATUSBAR`. Their headers do not call `template_header()`. Moodboard and Mixie Chat stay listed. The Texturing / Texture Paint workspace 3D header uses the same floating glass four-way shading strip (plus the shading popover) as Zen Mode (`view3d_header_filter`). Pinned by `tests/test_texturing_space_menu.py`.
