...
| **moodboard** | Reference boards + node canvas (Zen Mode: sliding VIEW_3D overlay drawer), catalog-driven generation sidebar (Image Gen, AI Render, Model Gen, Texture Gen, Scene Gen, Character Parts, Retopology, UV Unwrap, Mesh Segment, Auto Rig, Video Gen, World Labs + Queue), turnaround/multi-view image-to-3D, prompt Refine/Revert on every generation prompt, clipboard, scene recon, annotations, SAM3 character components, Gaussian splats |
