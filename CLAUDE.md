...
Foundation for the DB-driven moodboard tabs (all converted: Image Gen, Model Gen, Texture Gen, Retopology, UV Unwrap, Mesh Segment, Scene Recon — each with a legacy offline fallback).
...
- **`modules/common/generation_params/`** — schema-driven parameter engine: dynamically builds one PropertyGroup per (service, model) from catalog schemas, attached to **WindowManager** pointers (not Scene) so re-registration on catalog change is safe (no .blend/undo persistence; rebuilds run on a main-thread timer scheduled by the cache). `draw_service_params(layout, service, model)` renders widgets by schema `widget` kind honoring `order`/`group`/`visible`/`visible_if`; `collect_params(service, model)` returns visible params as a typed plain dict for payloads. Never hardcode param names here — key sets change from the DB without client releases. Design rationale in `core/engine.py` docstring. Two more layers live here: **`core/selector.py`** — capability mode selector (`get_service_enum_items()` Mode-dropdown items with LOADING/ERROR placeholders, `resolve_service_key()` mode-enum→valid service key, `draw_capability_selector()` Mode→Model→params rendering that returns False when the catalog isn't loaded so tabs fall back to legacy UI); and **`core/assemblers.py`** — service key → assembler registry (`assemble_payload(service, params, payload, model_slug)`) that merges `collect_params()` output into each service's EXISTING wire payload shape (`model_3d`→`payload["params"]`; `image_to_3d`→Hunyuan-Pro `sdk_params` PascalCase incl. `Model` version derived from the `hunyuan_pro_v3[.1]` slug; `hunyuan_rapid`→Rapid `sdk_params`; `hunyuan_texture_edit`→`sdk_params` (Prompt?) beside the FBX `file_bytes_b64`/`reference_image_bytes_b64`; `retopology`→`sdk_params` (PolygonType?/FaceLevel?) + top-level `post_process`; `retopology_tripo`→`tripo_params` (model "v2.0", quad-aware face_limit clamp, bake); `hunyuan_uv`/`hunyuan_part`→PascalCase `sdk_params` passthrough (no-op while paramless); unknown→`payload["params"]`). `resolve_model_slug()` (selector.py) maps a model enum value to a valid catalog slug with a hardcoded fallback. Wire contracts must never change here — the backend handlers keep their legacy payload shapes. Bespoke-queue services (`pbr_gen`, `mesh_segment`, `scene_reconstruction`, `mat_gen`) have no assembler entry: their Jobs build payloads from dataclass fields, so catalog params thread through the enqueue helpers' kwargs instead.
