...
- **Custom C++ editor spaces**: `space_mixie_chat`, `space_agent_bubble`, `space_mixar_properties`, `space_mixar_layers`, `space_mixar_assets`, plus the native chat renderer (markdown, thinking visualization, hit testing, thumbnails) and View3D overlays (Director, agent panel, gizmos). Authentication is cross-platform keyring + a local OAuth PKCE callback server. **Editor Type menu**: `SPACE_AGENT_BUBBLE` and the five texturing spaces (`MIXAR_LAYERS`, `MIXAR_PROPERTIES`, `MIXAR_ASSETS`, `BAKING`, `TEXTURE_SETS`) stay registered for saved layouts and `bl_space_type`, but `rna_Area_ui_type_itemf` hides them from the dropdown — same skip list as `SPACE_TOPBAR` / `SPACE_STATUSBAR`. Their headers do not call `template_header()`. Moodboard and Mixie Chat stay listed. The Texturing / Texture Paint workspace 3D header uses the same Mixar Solid/Rendered pills as Zen Mode (`view3d_header_filter`). Pinned by `tests/test_texturing_space_menu.py`.
