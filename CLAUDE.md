...
| **moodboard** | Reference boards + node canvas with shared editor/drawer chrome and catalog-gated node templates (Zen Mode: sliding VIEW_3D overlay drawer), catalog-driven canvas nodes and reusable generation forms (the standalone right sidebar is retired), turnaround/multi-view image-to-3D, prompt Refine/Revert on every generation prompt, clipboard, scene recon, annotations (drawer draw + erase), SAM3 character components, Gaussian splats |
