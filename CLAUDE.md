...
Dithered transparency count; fully occluded and reflection/refraction-only meshes do not.
Blended materials fail capture explicitly. `common/render_visibility/collect.py` separately
links the original meshes into a new collection; rendering never modifies collections.
`common/render_visibility/six_views.py` drives six orthographic captures, writes per-view
lists and their union, then separately moves the captured meshes into an exclusive collection
and saves a new file. It preserves ancestor transforms and existing source-view exclusions;
six-view visibility is an approximation of exterior membership, not a geometric shell test.
