...
| **moodboard** | Reference boards + node canvas with shared editor/drawer chrome and catalog-gated node templates (Zen Mode: sliding VIEW_3D overlay drawer opens at ~35% of the View3D; top-strip templates appear progressively as width allows; dropping Image to 3D onto a still attaches beside it), catalog-driven canvas nodes and reusable generation forms (the standalone right sidebar is retired), turnaround/multi-view image-to-3D, prompt Refine/Revert on every generation prompt, clipboard, scene recon, annotations (drawer draw + erase), SAM3 character components, Gaussian splats |
