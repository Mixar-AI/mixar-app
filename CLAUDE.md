...
| **moodboard** | Reference boards + node canvas with shared editor/drawer chrome and catalog-gated node templates (Zen Mode: sliding VIEW_3D overlay drawer), catalog-driven generation sidebar (Image Gen, AI Render, Model Gen, Texture Gen, Scene Gen, Character Parts, Retopology, UV Unwrap, Mesh Segment, Auto Rig, Video Gen, Video Upscale, World Labs + Queue), turnaround/multi-view image-to-3D, prompt Refine/Revert on every generation prompt, clipboard, scene recon, annotations (drawer draw + erase), SAM3 character components, Gaussian splats |
...
| **byok** / **local_models** | Bring-your-own-key provider settings (cloud catalog, OpenRouter, Codex, Local) plus the hosted agent model picker shared by the chat footer and the island (`MIXIE_CHAT_MT_agent_model`, `wm.mixar_agent_model_*`); zero-setup local llama.cpp runtime |
