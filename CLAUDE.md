...
**Scribble contract (`modules/scribble_mark/` + `space_mixie_chat/core/scribble.py`):**
one mode, two surfaces. Ink over the chat is handwriting and becomes TEXT in
the composer; ink over the 3D viewport points at the scene and becomes MARKS,
which the CLIENT resolves before anything leaves the machine — only the client
has the depsgraph, the view matrices and `scene.ray_cast`. The user chooses by
where they write, never by which button they pressed.

- **The two halves enter and leave together** (`core/scribble_mode.py`). The
  header control raises the C++ ink canvas (`mixie_chat_ink_visible`) BEFORE
  starting the freeze modal, which records that it is linked and follows the
  canvas down when C++ closes it (Esc / the X over the chat — paths Python
  never sees, so the modal polls the flag on its own timer). Every freeze
  exit lowers the canvas; the toggle and the send disarm both through
  `scribble_mode.disarm`, which converts what is still on the canvas FIRST
...
- **The ink of a freeze is read as MARKS or as ONE SKETCH** (`core/sketch.py`),
  and the reading is visible and flippable. A drawing — a road with cars and
  trees on the side — is chopped into one mark per pen-up pause, and sent as
  pointing marks it arrived as nine ground placement targets and no picture
  (the agent built a campfire). `sketch.read_intent` reads the drafts
  deterministically: a sketch needs `SKETCH_MIN_STROKES` strokes, at least
  `SKETCH_MIN_OPEN_STROKES` OPEN lines (three circles on the ground are three
  placement targets, not a drawing), and most marks reading as DRAWN — ink on
  nothing, a non-gesture `stroke`, a 3+-stroke doodle, or, in a drawing-sized
  session, a loop covering a sliver of a big surface such as a floor mesh
  (`SKETCH_CANVAS_MIN_STROKES` / `SKETCH_MAX_OBJECT_FRACTION`). Advisory,
  like the gesture labels: BOTH representations travel (`intent` +
  `intent_source`, and a `sketch` block of every stroke's kind, frame bbox
  and world path beside the marks), the hint pill says which reading is
  current (`MARK_HINT_SKETCH`), Tab flips it inside the freeze, and the
  `wm.mixar_mark_intent` dropdown beside the header count does the same
  outside it (SKIP_SAVE; reset on send, clear and file load; its update
  callback runs `marks.refresh_reading`, the ONE place the drafts are parsed
  for the pill — never the draw callback). Every raw stroke is stored
  normalized on its mark (`MARK_STROKE_MAX_POINTS`) and sampled into world
  space at commit (`resolve._strokes_world`: raycast, else the ground plane,
  with what most of it landed on); `payload._wire_mark` folds them into the
  sketch block and leaves per-mark copies off the wire. The annotated frame
  is drawn from the RAW INK (`annotate._mark_polylines`), not the gesture
  hull — a hull of a sketched car is a blob, and blobs were what the agent
  was shown. `MAX_MARKS_PER_TURN` is drawing-sized (32; eight refused the
  second half of the sketch) and a full stroke group COMMITS and starts
  another instead of dropping ink. `prepare_for_send` runs the payload
  through `payload.serialize` for real (sketch-first shedding: mark outlines,
  then stroke paths, then the shortest strokes; resolution never). Pinned by
  `tests/scribble_mark/test_scribble_sketch.py`.
...
- **Freezing is load-bearing, not decoration.** Arming captures the viewport
  ...
  user marked exactly the pixels the agent sees, lets several marks share one
  frame of reference ("move THIS over THERE"), and stops a stray orbit
  mid-gesture leaving marks that describe a view which no longer exists.
- **Resolution raycasts through the LIVE region** (`core/projection.py` via
  `view3d_utils`), never through the baked camera. Reconstructing a camera from
  a viewport means getting lens/sensor/ortho-scale conventions right; confining
  that to the camera keeps its failure mode cosmetic (the re-render is framed
  slightly differently) instead of silent and wrong (the agent edits the wrong
  object). The camera is derived from `RegionView3D.window_matrix`, NOT
  `SpaceView3D.lens` — the viewport's focal length is against a different
  notional sensor and copying it across visibly reframes.
- **`coverage` and `object_fraction` are different numbers and must not be
  conflated** — `coverage` is how much of the MARK is this object,
  `object_fraction` is how much of the OBJECT the mark covers (and drives
  `partial`). A scribble over one wall has high coverage and low
  object_fraction. An unmeasured `object_fraction` always reads as `partial`:
  "we could not measure it" reaching the agent as "the user selected all of it"
  is the difference between editing a roof and a house.
- **The vertex group is the handoff that matters** (`core/vertex_groups.py`).
  Coordinates make the agent write projection maths in a sandbox script; a
  named group is a noun it can address. Screen-space and front-facing, so "this
  wall" does not also select the far one; written against ORIGINAL vertices
  (groups are indexed by base vertex) and only for a PARTIAL selection.
- **Closed-loop detection uses total TURNING, not enclosed-area-vs-hull.**
  Any convex open arc encloses ~all of its own hull once you join its
  endpoints, so a 130-degree swipe read as a closed loop (`core/geometry.py`).
  Gesture reading is deterministic and never discards: the label is advisory
  and the raw polygon travels beside it.
- **Coordinates are `u` left→right, `v` bottom→top with NO y flip** — region
  coordinates are already bottom-up and this is the backend sculpt localizer's
  own convention, which is what lets a mark be a drop-in for `locate_region`.
  The PIL annotation pass DOES flip, because PIL rows are top-down.
- **The frame is sent twice, clean and annotated** (`core/annotate.py`).
  Burning the ink into the only image means every downstream generation
  faithfully reproduces the cyan loop — the same lesson `reference_image_roles`
  encodes by keeping `component_cutout` beside `selection_mask`. Annotated is
  queued first: under a tight attachment cap it is the one carrying information
  the agent cannot get any other way.
- **Marks are scene nouns, not message contents.** They persist in the .blend
  (`Scene.mixar_marks`, monotonic `mixar_mark_serial` that never reuses a
  number) and are KEPT after send, because a follow-up turn refers back to them
  and the groups and cameras they name are still live.
  `core.marks.get_marks()` is the sandbox re-read entry point, mirroring
  `sketch_blockout`'s `get_sketch_json()`. `WindowManager.mixar_mark_armed` is
  SKIP_SAVE by contract — a .blend reopened mid-freeze would be a file the user
  could not navigate.
- **A file load must never leave the viewport frozen** (`ui/mark_lifecycle.py`).
...
- The draw pass (`core/overlay.py`) READS ONLY — the live stroke is handed in
  ...
  lines single gestures without the user declaring it.
- Resolution runs at commit time on the main thread, so pointing at something
  adds no latency to sending. Nothing in the mark path waits on the network.
- Pinned by `tests/scribble_mark/` (geometry/gesture/payload/coverage as pure
  units; the Blender-facing halves as source-level contracts, including that
  `_stream_loop`'s positional argument chain stays in lockstep across its thread
  hand-off and its reconnect recursion).
...
- **Placement rides with the job** (`core/placement.py`): the agent-facing 3D operators (`mixie.image_to_3d_generate`, `mixie.hunyuan_generate` Pro/Rapid) take an optional `placement` JSON string — `{location, normal?, size?, rotation_z?, on?, mark?}` in world metres — and `make_model_rename_on_imported(..., placement=)` applies it right after `rename_generated_model` has normalized the import (base at the origin, facing -Y): scale so the longest side is `size`, yaw, tilt +Z onto `normal`, translate to `location`, all on the ROOT of the import so a Trellis assembly moves whole. Applied at IMPORT time because the job outlives the agent's turn — before this a model generated for a marked branch landed at the origin with "it still needs to be positioned", and the user had to come back to move it. Best-effort: a failure leaves the mesh at the origin, exactly where it would have been; the provenance is stamped on the mesh as `mixar_placement`. The matrix maths is pure (`placement_matrix`, Rodrigues +Z→normal) and pinned by `tests/test_generation_placement.py`.
